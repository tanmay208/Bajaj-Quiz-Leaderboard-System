...
1. **Initialize HTTP Client & JSON Mapper:** Sets up Jackson's `ObjectMapper` and Java's native `HttpClient`.
2. **Polling Loop:** Runs a loop 10 times (poll indexes 0 to 9).
...
4. **Mandatory Wait:** Pauses execution for 5 seconds before the next poll iteration.
5. **Leaderboard Construction:** Converts the aggregated `HashMap` into a `List` of `ParticipantScore` objects and sorts it by `totalScore` in descending order.
6. **Submission:** 
...
7. **Validation Output:** Parses the submission response and prints whether the submitted leaderboard was validated as correct or incorrect.
...
1. **Clone or Navigate** to the project directory:
   ```bash
   cd "d:\BAJAJ FINSERV HEALTH LIMITED\QuizLeaderboardSystem"
   ```

2. **Compile and Run** using Maven. The `exec-maven-plugin` is configured to run the main class directly.
   ```bash
   mvn clean compile exec:java
   ```

## Author / Registration Details
- **Registration Number Used:** `2024CS101`
- Target API base: `https://devapigw.vidalhealthtpa.com/srm-quiz-task`
